...
public class ManagerSteps {
    ...
    public void the_Manager_clicks_on_the_Tenant_Account_Management_button() {
        BasicRunner.messaging.accountMakerButton.click();
    }
    ...
    public void the_Manager_inputs_the_tenant_s_unit_number() {
        Select options = new Select(BasicRunner.accountMaker.selectUnits());
    ...
    public void the_Manager_should_see_a_success_message() {
        Assert.assertTrue(BasicRunner.accountMaker.alertMessageActice().getAttribute("class").contains("bg-success"));
    }

    @Then("The Submit button should be disabled")
    public void the_Submit_button_should_be_disabled() {
        Assert.assertEquals(BasicRunner.accountMaker.createNewAccountBtn.getAttribute("disabled"), "true");
    }

    @Then("The Submit button should be enabled")
    public void the_Submit_button_should_be_enabled() {
        Assert.assertNull(BasicRunner.accountMaker.createNewAccountBtnReenabled().getAttribute("enabled"));
    }

    @Then("The success message should disappear")
    public void the_success_message_should_disappear() {
        Assert.assertTrue(BasicRunner.accountMaker.alertMessageInactive().getAttribute("class").contains("hidden"));
    }

    @When("The Manager clicks on View Unit List")
    public void the_Manager_clicks_on_View_Unit_List() {
        BasicRunner.messaging.unitListButton.click();
    }
    ...
    public void the_manager_clicks_on_the_plus_sign_of_Type_apartments(Integer int1) {
         if (int1 == 2) {
            BasicRunner.unitList.normalPlus.click();
        }else if(int1 == 3){
            BasicRunner.unitList.deluxPlus.click();
        }
    }
    ...
    public void the_manager_should_see_the_list_of_Type_apartments(Integer int1) throws InterruptedException {
        ...
        if(int1 == 2){
           WebElement col =  BasicRunner.unitList.normalLastRowCol1;
           Assert.assertTrue(col.getAttribute("innerHTML").contains("210"));
        } else if (int1 == 3) {
            WebElement col =  BasicRunner.unitList.deluxeLastRowCol1;
    ...
    public void the_list_of_Type_apartments_should_be_hidden(Integer int1) {
        if (int1 == 2) {
            WebElement table = BasicRunner.unitList.typeNormalTableClosed();
            Assert.assertTrue(table.getSize().getHeight() < 200);
        }else if(int1 == 3){
            WebElement table = BasicRunner.unitList.typeDeluxeTableClosed();
            Assert.assertTrue(table.getSize().getHeight() < 200);
        }
    }
    ...
    public void the_manager_sees_recent_messages() throws InterruptedException{
        TimeUnit.SECONDS.sleep(2);
        WebElement el =  BasicRunner.messaging.recentMessageRow1Col1;
    ...
    public void the_manager_clicks_on_the_plus_sign() throws InterruptedException{
            TimeUnit.SECONDS.sleep(2);
            BasicRunner.messaging.plusBtn.click();
    }
    ...
    public void the_manager_should_see_a_list_of_past_messages() throws InterruptedException {
        WebElement el =  BasicRunner.messaging.pastMessagesRow1Col1;
        Assert.assertNotNull(el.getAttribute("innerHTML"));
    }
    ...
    public void the_new_message_should_be_displayed_at_the_top_of_recent_messages() {
        String msg = BasicRunner.messaging.recentFirstRowMessage.getAttribute("innerHTML");
    ...
    public void the_manager_clicks_on_the_close_popup_btn() {
        BasicRunner.messaging.closeBtn.click();
    }
}